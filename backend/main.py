...
while True:
    ...
    data = load_json("backend/data/save_slots/" + str(plr_slot) + ".json")
    if data:
        print("Data loaded!")
        break
    else:
        print("No save slot found!")
        game.run("look", plr_slot)
        print("Created new save slot!")
        break

while True:
    cmd = input(">") or "look"
    result = game.run(cmd, plr_slot)
    ...
    if split_cmd[0] == "load" and split_cmd[1]:
        if load_json("backend/data/save_slots/" + str(plr_slot) + ".json"):
